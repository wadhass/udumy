...
//                   </div>
//                   <div className={`overflow-hidden transitions-all duration-300 ${openSection[index] ? 'max-h-96' : 'max-h-0'}`}>
...
//             <button className='md:mt-6 mt-4 w-full py-3 bg-blue-600 text-white rounded font-medium'>
...
const CourseDetails = () => {
  ...
  const fetchCourseData = async () => {
    try {
      const findCourse = allCourses.find(course => course._id === id?.trim());
      setCourseData(findCourse);
    } catch (error) {
  ...
  useEffect(() => {
    if (courseData && courseData.enrolledStudents) {
      const userEnrolled = courseData.enrolledStudents.includes('userId'); // Replace 'userId' with actual user ID logic
      setAlreadyEnrolled(userEnrolled);
    }
  }, [courseData]);
